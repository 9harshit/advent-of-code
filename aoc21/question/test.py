...
file_name_test = "aoc3.txt"
file_name_challenge = "aoc3.txt"
...
def transform_data(list_zeros, list_ones):
    # Used in partOne
    final_ganmma = line_toString_bit(
        [1 if x > y else 0 for x, y in zip(list_ones, list_zeros)]
    )
    final_epsilon = line_toString_bit(
        [1 if x < y else 0 for x, y in zip(list_ones, list_zeros)]
    )
...
def get_filter_criteria(fd, max):
    ...
    for i in range(line_lenght):
        if len(all_lines) > 1:
            ...
            if max:
                all_lines = list(
                    filter(lambda arraybit: arraybit[i] == mc_bit, all_lines)
                )  # Filter the new list
            else:
...
print(
    f"This is the result - BIT STRING - {objetiveOxigen_bitarray} - VALUE - {objetiveOxigen_value} "
)
...
print(
    f"This is the result - BIT STRING - {objetiveC02_bitarray} - VALUE - {objetiveC02_value} "
)
